...
# Show Uninstallable programs
complete -fc cargo -n '__fish_seen_subcommand_from uninstall' -a '(cargo install --list | grep "^[^ ]" | awk \'{print $1, substr($2,1,length($2) -1)}\' | string replace \' \' \\t)'
